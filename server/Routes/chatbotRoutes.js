...
const getIndianCropRecommendations = (month, temperature, rainfall, humidity, location) => {
    ...
    if (season === 'Kharif') {
        ...
        if (temperature > 28 && rainfall > 100) {
            crops = ['Rice', 'Cotton', 'Sugarcane', 'Maize', 'Jowar', 'Bajra', 'Tur (Arhar)', 'Moong', 'Urad', 'Groundnut'];
        } else if (temperature > 25) {
            crops = ['Maize', 'Jowar', 'Bajra', 'Cotton', 'Tur (Arhar)', 'Moong', 'Sesame'];
        } else {
        ...
        if (isNorthIndia) {
            crops = ['Wheat', 'Barley', 'Gram (Chana)', 'Peas', 'Lentil (Masoor)', 'Mustard', 'Potato', 'Onion'];
        } else {
            crops = ['Wheat', 'Jowar', 'Bajra', 'Gram (Chana)', 'Sunflower', 'Coriander', 'Cumin', 'Fenugreek'];
        }
        if (temperature < 20) {
        ...
        if (temperature > 35) {
            crops = ['Watermelon', 'Muskmelon', 'Cucumber', 'Fodder crops (Jowar, Maize)'];
        } else {
            crops = ['Maize', 'Jowar', 'Bajra', 'Moong', 'Urad', 'Sunflower', 'Sesame'];
        }
        if (rainfall > 50) {
...
const getGenericClimateData = (month, state) => {
    ...
    if (state) {
        ...
        } else if (stateLower.includes('kerala') || stateLower.includes('karnataka')) {
...
const getRuleBasedResponse = (message, location, weather) => {
    ...
    if (msg.includes('crop') || msg.includes('grow') || msg.includes('plant') || msg.includes('season')) {
        const currentMonth = new Date().getMonth() + 1;
        ...
        const { crops, seasonInfo } = getIndianCropRecommendations(currentMonth, temp, rainfall, humidity, location);
        ...
        response += '. Consider local soil conditions, water availability, and market demand when making final decisions.';
        return response;
    }
    
    // Soil quality
    if (msg.includes('soil') || msg.includes('quality') || msg.includes('fertility')) {
        return seasonContext + 'To improve soil quality: 1) Add organic compost regularly, 2) Practice crop rotation, 3) Test soil pH (ideal range 6.0-7.0), 4) Use cover crops, 5) Avoid over-tilling. Regular soil testing helps monitor nutrient levels.';
    }
    
    // Irrigation
    if (msg.includes('irrigation') || msg.includes('water') || msg.includes('watering')) {
        return seasonContext + 'Best irrigation practices: 1) Water early morning or evening, 2) Use drip irrigation for efficiency, 3) Monitor soil moisture, 4) Mulch to retain moisture, 5) Adjust watering based on weather and crop stage.';
    }
    
    // Pest control
    if (msg.includes('pest') || msg.includes('insect') || msg.includes('bug') || msg.includes('disease')) {
        return seasonContext + 'Pest management tips: 1) Use integrated pest management (IPM), 2) Encourage beneficial insects, 3) Rotate crops annually, 4) Remove infected plants promptly, 5) Use organic pesticides when necessary, 6) Monitor crops regularly.';
    }
    
    // Fertilizers
    if (msg.includes('fertilizer') || msg.includes('nutrient') || msg.includes('nitrogen') || msg.includes('phosphorus')) {
        return seasonContext + 'Fertilizer recommendations: 1) Test soil first, 2) Use balanced NPK fertilizers, 3) Apply organic compost, 4) Consider slow-release fertilizers, 5) Follow crop-specific nutrient requirements, 6) Avoid over-fertilization.';
    }
    ...
    if (msg.includes('weather') || msg.includes('rain') || msg.includes('temperature')) {
        const weatherInfo = weather 
            ...
            : '';
        return seasonContext + weatherInfo + 'Monitor weather forecasts for farming decisions. Protect crops from extreme weather, adjust irrigation based on rainfall, and plan planting/harvesting around weather patterns.';
    }
    
    // Default response
    return seasonContext + 'I can help you with crop recommendations, soil management, irrigation, pest control, fertilizers, and weather-related farming advice. What specific farming topic would you like to know about?';
};
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                const predResponse = await axios.post(`${ML_SERVICE_URL}/predict`, predictionParams, {
        ...
        if (mlResponse && mlResponse.message) {
            ...
            const ruleBasedResponse = await getRuleBasedResponse(message, req.user?.id, req.user?.type);
            return res.json({
        ...
        try {
            const ruleBasedResponse = await getRuleBasedResponse(req.body.message || 'help', req.user?.id, req.user?.type);
            res.json({
                message: ruleBasedResponse
            });
        } catch (fallbackError) {
            // Final fallback
            res.json({ 
                message: 'I\'m here to help with your farming questions! You can ask me about crop recommendations, soil management, irrigation, pest control, fertilizers, and weather-related farming advice.'
            });
        }
    }
});

...
const generateToken = (user, type) => {
  try {
    return jwt.sign({ user: { id: user.id, type } }, process.env.JWT_SECRET, {
      expiresIn: '24h',
    });
  } catch (err) {
...
exports.register = async (req, res, next) => {
  try {
    ...
    if (userType === 'farmer') {
      ...
      const { name, email, password, phoneNumber, type, state, city, address } =
        req.body;
      ...
      try {
        ...
        if (!validTypes.includes(type)) {
          ...
          return res.status(400).json({
            ...
            message: 'Validation Error',
            errors: [
              `Invalid consumer type. Must be one of: ${validTypes.join(', ')}`,
            ],
          });
        }
  ...
  try {
    ...
    if (userType === 'farmer') {
      if (!phoneNumber || !password) {
        console.log(
          'Farmer login validation failed: Missing phone or password'
        );
        return res
          .status(400)
          .json({
      ...
      // Explicitly check for consumer
      if (!email || !password) {
        console.log(
          'Consumer login validation failed: Missing email or password'
        );
        return res
          .status(400)
          .json({
      ...
      console.log(
        'Login failed: Invalid user type derived from path',
        req.path
      );
      return res
        .status(400)
    ...
    if (userType === 'farmer') {
      ...
      // Assuming valid userType is either farmer or consumer after validation
      Model = Consumer;
      query = { email };
    }

...
const migrateConsumerStateCity = async () => {
  try {
    console.log(
      'Starting migration to add state and city fields to consumers...'
    );
    ...
    for (const consumer of consumersToUpdate) {
      ...
      await Consumer.findByIdAndUpdate(consumer._id, {
        $set: {
          ...
          address:
            consumer.address || consumer.location || 'Address not provided',
        },
      });

      console.log(
        `Updated consumer ${consumer.name} (${consumer.email}) with state: ${defaultState}, city: ${defaultCity}`
      );
    }

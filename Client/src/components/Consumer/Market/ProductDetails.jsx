  ...
  crops: [
    {
      ...
      quantity: 1000,
      description:
        'High-quality wheat grain suitable for premium flour production. Cultivated using modern farming techniques with optimal nutrient management.',
  ...
  agriWaste: [
    {
      ...
      quantity: 5000,
      description:
        'Clean and processed rice husk suitable for biomass energy production. Low moisture content and high calorific value.',
      availableFrom: '2025-02-01',
      farmer: {
        name: 'David Clark',
        location: 'Bihar',
...
const ProductDetails = () => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        const response = await axios.get(
          `/api/consumer/market/products/${productId}`
        );
        ...
        const allProducts = [
          ...dummyProducts.crops,
          ...dummyProducts.agriWaste,
        ];
        const dummyProduct = allProducts.find(
  ...
  const getProductImage = product => {
    if (product.type === 'crops' || product.type === 'agriWaste') {
      return (
        productImages[product.type][product.productName] ||
        (product.type === 'crops'
          ? '/images/market/crops.jpg'
          : '/images/market/agri-waste.jpeg')
      );
    }
            ...
            <Paper elevation={3} sx={{ p: 3 }}>
              ...
              <Box display="flex" flexDirection="column" gap={2} mb={3}>
                <Box display="flex" alignItems="center" gap={1}>
                  <Person />
                  <Typography>
                    {product.farmer.name} ({product.farmer.experience}{' '}
                    experience)
                  </Typography>
                </Box>
                ...
                <Box display="flex" alignItems="center" gap={1}>
                  <LocationOn />
                  <Typography>{product.farmer.location}</Typography>
                </Box>

                <Box display="flex" alignItems="center" gap={1}>
                  <Phone />
                  <Typography>{product.farmer.phone}</Typography>
                </Box>

                <Box display="flex" alignItems="center" gap={1}>
                  <Email />
                  <Typography>{product.farmer.email}</Typography>
                </Box>
              </Box>

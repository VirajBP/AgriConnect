...
const ConsumerDashboard = () => {
  ...
  const getChartData = period => {
    ...
    const chartData = months.map(month => {
      ...
      if (dashboardData?.monthlySpending) {
        const existingMonth = dashboardData.monthlySpending.find(
  ...
  return (
    <div className="consumer-dashboard">
      ...
      />
      <div
        className={`dashboard-content ${isSidebarCollapsed ? 'sidebar-collapsed' : ''} ${isDarkMode ? 'dark' : ''}`}
      >
        ...
        <div className="dashboard-main">
                  ...
                  <button
                    ...
                    disabled={
                      ...
                      (!dashboardData?.monthlySpending ||
                        dashboardData.monthlySpending.length < 1)
                    }
                  >
                    {period}
                  </button>
          ...
          <div
            className={`${isDarkMode ? 'dark recent-activity' : 'recent-activity'}`}
          >
            <div className="section-header">
            ...
            <div className="activity-list">
              {dashboardData?.recentOrders?.map(order => (
                <div
                  key={order._id}
                  className={`${isDarkMode ? 'dark activity-item' : 'activity-item'}`}
                >
                  ...
                  <div className="activity-details">
                    ...
                    {order.productName && (
                      <p className="activity-items">{order.productName}</p>
                    )}
